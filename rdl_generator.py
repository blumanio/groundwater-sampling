"""
rdl_generator.py  — ACR di Reggiani Albertino S.P.A.
Place in: server/rdl_generator.py

Replicates exact XLS structure of official RDL templates:
  - bg_idx=22 → RGB (192,192,192) gray  → section headers
  - bg_idx=51 → RGB (255,204,0)  yellow → NOTE LAVORI / NOTE SICUREZZA

Usage via stdin (recommended, avoids Windows escaping):
  echo '{"committente":"ENI REWIND",...}' | python rdl_generator.py /path/to/assets
Output: base64-encoded .xlsx to stdout
"""
import openpyxl
...
_BM = {0: None, 1: 'hair', 2: 'thin', 7: 'hair'}
def _s(style): return Side(border_style=style) if style else Side()
def B(l=0,r=0,t=0,b=0):
    return Border(left=_s(_BM.get(l)), right=_s(_BM.get(r)), top=_s(_BM.get(t)), bottom=_s(_BM.get(b)))

def sc(ws,r,c,v,bold=False,sz=10,h='general',wrap=False,bl=0,br=0,bt=0,bb=0,fill=None):
    cell=ws.cell(r,c)
    try: cell.value=v
    except: pass
    try:
...
def generate_rdl(data, assets_dir=None):
    ...
    # R3: DESCRIZIONE LAVORI — GRAY
    sc(ws,3,1,'DESCRIZIONE LAVORI',bold=True,sz=9,h='left',bl=2,br=0,bt=2,bb=2,fill=GRAY_FILL)
    for c in range(2,10): sc(ws,3,c,'',bl=0,br=0,bt=2,bb=2,fill=GRAY_FILL)
    ...
    noleggio=data.get('noleggio',[]); mat_opera=data.get('materiali_opera',[])
    for i in range(7):
        r=19+i; m=mat_opera[i] if i<len(mat_opera) else {}
    ...
    for r in [48,49]: M(ws,r,r,8,10,'',bold=True,sz=8,h='center',bl=1,br=1,bt=0,bb=2)
    for r in range(47,50):
        M(ws,r,r,1,2,'',sz=9,h='center',bl=2,br=1,bt=7,bb=7 if r<49 else 2)
        sc(ws,r,3,'',sz=8,bl=0,br=1,bt=7,bb=7 if r<49 else 2)
        sc(ws,r,4,'',sz=11,bl=1,br=1,bt=7,bb=7 if r<49 else 2)
        sc(ws,r,5,'',sz=10,bl=1,br=2,bt=0,bb=7 if r<49 else 2)

    if assets_dir:
        for fname,anchor,w,h in [('logo_eni.png','A1',55,50),('logo_acr_strip.jpg','A2',170,35)]:
